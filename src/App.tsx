...
import { 
  ...
  Moon, Sun, Sparkles, ChevronDown, Search, ArrowUpRight
} from 'lucide-react';
...
function AdminLayout() {
  ...
  // Sincronización del Modo Noche/Día en el elemento raíz
  useEffect(() => {
    if (isDarkMode) {
  ...
  return (
    <div className={`h-screen w-screen overflow-hidden flex flex-col transition-colors duration-500 font-sans ${isDarkMode ? 'bg-[#080A0F] text-slate-100' : 'bg-slate-100 text-slate-900'}`}>
      
      {/* Resplandor Ambiental Luminoso (Basado en el color de la tienda) */}
      <div 
        className="fixed -top-24 left-1/4 w-[500px] h-[500px] rounded-full blur-[140px] pointer-events-none opacity-20 dark:opacity-10 transition-all duration-700 z-0"
        style={{ backgroundColor: accentColor }}
      />

      {/* Contenedor Bento Flotante Principal */}
      <div className="flex-1 flex overflow-hidden p-0 sm:p-3 lg:p-4 gap-4 relative z-10">
        
        {/* ========================================================= */}
        {/* 1. SIDEBAR LATERAL FLOTANTE (Escritorio - Estilo Helios)  */}
        {/* ========================================================= */}
        <aside className="hidden md:flex flex-col justify-between w-64 lg:w-72 shrink-0 rounded-[2rem] bg-white/80 dark:bg-[#11141D]/90 backdrop-blur-2xl border border-slate-200/80 dark:border-white/5 p-5 shadow-2xl transition-all">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3.5 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-300 group ${
                      isActive 
                        ? 'text-white shadow-lg shadow-black/20' 
                        : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white'
                    }`}
                    style={isActive ? { 
                      background: `linear-gradient(135deg, ${accentColor} 0%, #0f172a 120%)`
                    } : {}}
                  >
          ...
          <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-white/5">
            ...
            <div className="flex items-center justify-between p-2.5 rounded-2xl bg-slate-50 dark:bg-[#181B26] border border-slate-200/60 dark:border-white/5">
              <div className="flex items-center gap-2.5 min-w-0 pr-1">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs font-bold truncate">{activeStore?.name}</p>
                </div>
              </div>
            ...
            {/* Selector de Tiendas si hay múltiples */}
            ...
            <button 
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider text-rose-500 hover:bg-rose-500/10 transition-colors"
            >
        ...
        {/* ========================================================= */}
        {/* 2. ÁREA PRINCIPAL BENTO GRID (Scroll Interno)             */}
        {/* ========================================================= */}
        <div className="flex-1 flex flex-col h-full overflow-hidden rounded-none sm:rounded-[2rem] bg-white/70 dark:bg-[#11141D]/70 backdrop-blur-2xl border-0 sm:border border-slate-200/80 dark:border-white/5 shadow-2xl">
          
          {/* Header Superior Integrado */}
          <header className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-slate-200 dark:border-white/5 shrink-0">
            
            <div className="flex items-center gap-3">
              <button 
                onClick={() => setIsMobileAdminMenuOpen(true)}
                className="md:hidden w-10 h-10 rounded-2xl bg-white dark:bg-[#181B26] border border-slate-200 dark:border-white/5 flex items-center justify-center shadow-sm active:scale-95 transition-transform"
              >
                <Menu className="w-5 h-5" />
              </button>

              <div>
                <h2 className="text-lg sm:text-xl font-black uppercase tracking-tight truncate max-w-[180px] sm:max-w-md">
            ...
            {/* Controles de Acción: Tema, Catálogo y Avatar */}
            <div className="flex items-center gap-2 sm:gap-3">
              
              {/* Botón Modo Noche/Día */}
              <button 
                onClick={() => setIsDarkMode(!isDarkMode)}
                className="p-2.5 rounded-full bg-white dark:bg-[#181B26] border border-slate-200 dark:border-white/5 hover:scale-105 active:scale-95 transition-all text-slate-600 dark:text-yellow-400 shadow-sm"
                title={isDarkMode ? 'Cambiar a modo claro' : 'Cambiar a modo oscuro'}
              >
                {isDarkMode ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
              </button>

              {/* Botón Ver Catálogo Píldora */}
              <a 
                href={publicUrl} 
                target="_blank" 
                rel="noopener noreferrer" 
              ...
              <button 
                onClick={() => catalog.logout()}
                className="p-2.5 rounded-full bg-rose-50 dark:bg-rose-500/10 text-rose-500 md:hidden"
                title="Cerrar Sesión"
              >
                <LogOut className="w-4 h-4" />
              </button>
          ...
          {/* Contenedor de la Vista Activa con Scroll Limpio */}
          ...
          {/* Footer Corporativo Adaptado */}
          <footer className="hidden md:flex w-full items-center justify-between px-6 py-3 border-t border-slate-200 dark:border-white/5 bg-white/40 dark:bg-[#11141D]/40 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 shrink-0">
            ...
            >
              <div>
                <div className="flex justify-between items-center pb-5 border-b border-slate-100 dark:border-white/5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-black text-xs uppercase tracking-wider">CataSystem</span>
                  </div>
                  <button 
                    onClick={() => setIsMobileAdminMenuOpen(false)} 
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-3 transition-colors ${
                          isActive ? 'text-white shadow-md' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5'
                        }`}
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-white/5 space-y-3">
                <button 
                  ...
                  className="w-full py-3.5 bg-rose-50 dark:bg-rose-500/10 text-rose-600 rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 shadow-sm"
                >
      ...
      {/* ========================================================= */}
      {/* 4. BARRA INFERIOR FIJA MÓVIL (Glassmorphism)               */}
      {/* ========================================================= */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-[#0E1118]/80 backdrop-blur-xl border-t border-slate-200 dark:border-white/5 z-40 px-4 py-2 flex justify-between gap-1 pb-safe">
        <button 
          onClick={() => setCurrentView('admin-orders')} 
          className={`flex flex-col items-center justify-center gap-1 p-2 rounded-xl flex-1 transition-colors ${currentView === 'admin-orders' ? 'text-white' : 'text-slate-400'}`}
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center gap-1 p-2 rounded-xl flex-1 transition-colors ${currentView === 'admin-products' ? 'text-white' : 'text-slate-400'}`}
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center gap-1 p-2 rounded-xl flex-1 transition-colors ${currentView === 'admin-categories' ? 'text-white' : 'text-slate-400'}`}
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center gap-1 p-2 rounded-xl flex-1 transition-colors ${currentView === 'admin-appearance' ? 'text-white' : 'text-slate-400'}`}
