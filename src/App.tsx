...
import { Settings, ExternalLink, LayoutDashboard, LayoutList, LogOut, Store as StoreIcon, Palette, Paintbrush, Receipt } from 'lucide-react';
...
import { AdminOrders } from './components/AdminOrders'; // <-- Importamos la nueva pantalla
import { Login } from './components/Login';

function AdminLayout() {
  // Añadimos 'admin-orders' como la vista por defecto para que la veas de una vez
  ...
  // Filtramos los pedidos para esta tienda
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col overflow-hidden font-sans">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-40 px-2 py-3 flex justify-around overflow-x-auto gap-2">
          <button onClick={() => setCurrentView('admin-orders')} className={`flex flex-col items-center gap-1 p-2 text-xs font-medium min-w-[60px] ${currentView === 'admin-orders' ? 'text-black' : 'text-slate-500'}`}><Receipt className="w-5 h-5" /> Pedidos</button>
