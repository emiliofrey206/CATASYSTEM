...
import { Settings, ExternalLink, LayoutDashboard, LayoutList, LogOut, Store as StoreIcon, Palette, Paintbrush } from 'lucide-react';
...
import { AdminAppearance } from './components/AdminAppearance'; // NUEVO IMPORT
import { Login } from './components/Login';

function AdminLayout() {
  // NUEVO: Agregamos 'admin-appearance' a los estados de la vista
  const [currentView, setCurrentView] = useState<'admin-products' | 'admin-categories' | 'admin-stores' | 'admin-colors' | 'admin-appearance'>('admin-products');
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col overflow-hidden font-sans">
      <header className="flex items-center justify-between px-6 sm:px-8 py-4 shrink-0 bg-white border-b border-slate-200 z-10 relative">
        <div className="flex items-center gap-3 flex-1">
          <div className="w-10 h-10 bg-black rounded-xl flex items-center justify-center shrink-0 shadow-sm"><Settings className="w-6 h-6 text-white" /></div>
        ...
        <div className="flex items-center justify-end gap-4 flex-1">
          ...
          <a href={publicUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-blue-600 bg-blue-50 px-4 py-2 rounded-xl flex items-center gap-2"><ExternalLink className="w-4 h-4" /> <span className="hidden lg:inline">Ver Catálogo</span></a>
          <button onClick={() => catalog.logout()} className="text-sm font-semibold text-slate-600 hover:text-red-600 hover:bg-red-50 px-4 py-2 rounded-xl hidden sm:flex"><LogOut className="w-4 h-4" /> Salir</button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* MÓVIL MENU */}
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-40 px-2 py-3 flex justify-around overflow-x-auto gap-2">
          <button onClick={() => setCurrentView('admin-stores')} className={`flex flex-col items-center gap-1 p-2 text-xs font-medium min-w-[60px] ${currentView === 'admin-stores' ? 'text-black' : 'text-slate-500'}`}><StoreIcon className="w-5 h-5" /> Tiendas</button>
