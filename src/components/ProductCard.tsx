...
export function ProductCard({ product }: ProductCardProps) {
  ...
  // Recopilar todas las imágenes únicas del producto (Principal + Variantes)
  ...
  // Bloquear el scroll de la página de fondo cuando la galería está abierta
  useEffect(() => {
    if (isGalleryOpen) {
  ...
  return (
    <>
      <div className="bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl hover:border-slate-200 transition-all duration-300 flex flex-col h-full group">
        ...
        {/* SECCIÓN VISUAL (Ahora Cuadrada y con Zoom) */}
        <div 
          onClick={openGallery}
          className="relative aspect-square bg-slate-50 overflow-hidden shrink-0 cursor-pointer"
        >
          {activeImage ? (
            <>
              <img 
              ...
              />
              {/* Ícono de ampliar al pasar el ratón */}
      ...
      {/* MODAL DE GALERÍA DE IMÁGENES (LIGHTBOX) */}
      <AnimatePresence>
        {isGalleryOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md" onClick={() => setIsGalleryOpen(false)}>
            ...
            {allImages.length > 1 && (
              <>
                <button 
                  onClick={prevImage}
                  className="absolute left-4 sm:left-8 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors z-50"
                >
                  <ChevronLeft className="w-8 h-8" />
                </button>
                <button 
                  onClick={nextImage}
                  className="absolute right-4 sm:right-8 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors z-50"
                >
            ...
            <motion.div
              key={galleryIndex} // Hace que se anime cada vez que cambia el índice
            ...
            >
              <img 
                ...
                alt={`${product.name} - Galería`} 
                className="w-full h-full object-contain max-h-[80vh] rounded-xl shadow-2xl" 
              />
              ...
              {allImages.length > 1 && (
                <div className="flex gap-2 mt-6">
                  {allImages.map((_, idx) => (
                    <div 
                      key={idx} 
                      className={`w-2.5 h-2.5 rounded-full transition-all ${idx === galleryIndex ? 'bg-white scale-125' : 'bg-white/30'}`}
                    />
                  ))}
                </div>
              )}
